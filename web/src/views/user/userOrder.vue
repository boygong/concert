    ...
    <div>
        ...
        <a-table :columns="columns" :data-source="orders" :pagination="false" row-key="orderId"
            ...
            <template v-slot:action="{ record }">
                ...
                <!-- 拒绝订单按钮 -->
                <a-button type="primary" danger ghost style="margin-left: 8px;" @click="showRejectModal(record)">取消订单</a-button>
            </template>
        </a-table>

         <!-- 拒绝理由输入弹窗 -->
         <a-modal v-model:visible="rejectModalVisible" title="取消订单" @cancel="closeRejectModal" @ok="cancelOrder" ok-text="确认"
            cancel-text="取消">
            <a-form :model="rejectionForm" :label-col="{ span: 6 }" :wrapper-col="{ span: 18 }">
                <a-form-item label="取消理由">
                    <a-input v-model:value="rejectionForm.cancelReason" placeholder="请输入取消理由" />
                </a-form-item>
            </a-form>
        </a-modal>
...
export default defineComponent({
    ...
    setup() {
        ...
        const cancelOrder = async () => {
            const cancelReason = rejectionForm.value.cancelReason;
            if (!cancelReason) {
            ...
            try {
                const response = await axios.post("/order/order/cancelOrder", {
